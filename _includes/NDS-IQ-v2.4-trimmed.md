# NDS IQ — building UI with the National Design System (instructions v2.4)
...
**`NDS_ROOT`** — the flat contents of one NDS template release. Default: `.nds/` at project root, gitignored. The path is correct when `NDS_ROOT/_site/` resolves directly; never keep the zip's versioned wrapper. Read-only.
...
|---|---|
| NDS has no X / use native X | search catalog `use_when` (§Build) |
...
|---|---|---|
| JS/CSS banners disagree | inconsistent runtime | correct release |
| both banners lack `Version:` | release unknown | release |
...
If the dev says a found runtime is legacy, ignore it: treat setup as new and assess its pages as prior NDS work.
...
3. **Copy canonical markup verbatim. Never invent it.** Copy the `lang-html` block from `_source/<folder>/<name>.md`; use `_site/<folder>/<name>.html` as the built reference. Preserve structure, classes, `data-*`, and ARIA.
   ...
   | CSP knob | Under strict `style-src`, move inline `style="--…"` values to a project-scoped class in an allowed `<style>` |
...
3. If `NDS_ROOT/_source/` is absent, download the SAME tag's source zip to a temp folder outside the project and copy available `_js`, `_sass`, `components`, `utilities`, `layout`, `ui-shell`, `core`, `templates`, `examples`, `_data/content` into `NDS_ROOT/_source/`.
...
- Older releases remain valid canon. Populate `_source/` from the matching tag only; never substitute newer source. Missing docs/banners are reported, not blockers.

## Plan

NDS is a UI layer; the project must already exist and serve. A fresh scaffold qualifies. No project → report and stop.

**Inventory first.** List routes, layouts, shared partials, pages/views, and legacy UI libraries. Each client-side view gets its own row. Map every page through the §Build composition cascade and record its per-page chrome shape (`full`, `console`, `minimal`).
...
Inspect every globally loaded stylesheet for element selectors (`body`, `h1`, `a`, `input`, etc.). Record hits and how the approved porting strategy isolates NDS pages from them.

- **Greenfield:** plan only pages the dev named.
...
- **Legacy NDS:** rebuild clean in place; old work is content/flow/data reference only. Remove its NDS footprint through the approved plan. Rollback is git.
- **Second runtime:** only by explicit dev decision; use parallel files plus a second assets folder.
...
- **Pacing:** `gate-by-gate` (default) or `whole plan`. Whole-plan mode uses file defaults for unresolved questions, verifies each page, and leaves completed rows `Awaiting Verification` until dev confirmation.
- When all rows are verified, retire the plan. Recreate it for a new multi-page effort or re-audit from current state.
...
2. **Master layout** — copy the complete `<body>` from a built `_site/` page matching the planned shape; never recreate it from prose. Use `_source/layout/page-shell.md` for shapes/modifiers. Older templates: choose the matching built page by inspection and report the missing reference.
...
**Copied chrome ships by default.** Keep self-contained chrome/widgets, including DGA stamp and dark-mode toggle; record removable items as plan checkboxes. Never infer affiliation.
...
1. Matching DGA template → copy `_source/templates/<name>.md`; swap content only.
...
If NDS truly has no needed surface, direct code is allowed with a one-line comment naming what was checked and an `NDS-REPORT.md` entry.
...
- **Visual:** inspect desktop and mobile screenshots for spacing, icons, width/sticky behavior, dark mode, and overall coherence. Measurements alone are not visual verification.

Drive both passes with an existing headless browser/harness (Playwright, Puppeteer, e2e harness, headless Chrome). Do not install one into the project or modify its lockfile. Behavioral = console + audit; visual = screenshots you actually inspect. You may claim "cannot see the page" only after this attempt fails; report the exact failure.

If headless fails, use the first available fallback and report what remains unverified:
...
For rules updates, compare the project-root `NDS-IQ.md` with raw main, never `NDS_ROOT/NDS-IQ.md`. Any difference means a newer revision. An explicit "update the rules" request skips comparison.
...
3. **Sweep pages** — read every relevant `### Migrating from` section in `CHANGELOG.md`; add affected work to `NDS-PLAN.md`, map it to pages, execute, and report. Also report useful `Added` / `Changed` / `Fixed` items for dev choice.
4. **Update this file** — download raw main `NDS-IQ.md` with curl/the stack HTTP client, not a web-fetch tool. Accept only if line 1 starts `# NDS IQ`; otherwise discard and retry once. Replace the project-root copy whole, leave the anchor untouched, then reread the file before continuing.

## Facts the docs assume

- RTL: Arabic uses `<html dir="rtl" lang="ar">`; LTR uses `dir="ltr"`. No second stylesheet.
- Dark mode: `data-theme~="dark"` on `<html>`.
- Icons: registered inline `nds-hgi-*` set + HGI font classes (§Build).

## Reference index

| Path | Holds |
|---|---|
| `_source/components/*.md` | canonical component markup/usage |
| `_source/utilities/*.md`, `_source/layout/*.md`, `_source/ui-shell/*.md` | utilities, shell/layout primitives, chrome |
| `_source/core/*.md` | runtime APIs/lifecycle/request |
| `_source/templates/*.md`, `_source/examples/*.md` | full-page sources; built twins under `_site/` |
| `_source/_data/content/*.yml` | component/template/example/icon catalogs |
| `_source/_js/nds-<name>.js` | component JS banners/behavior |
| `_source/_sass/components/_<name>.scss` | component styling/knobs |
| `_source/_sass/tokens/`, `_source/_sass/themes/_dga.scss` | tokens/themes |
| `_site/**/*.html` | built human-readable twins |
...
**Migrating from pasted instructions (v6 and earlier):** install this file + anchor, carry over the two path values, delete the old pasted instruction block, then run §Plan conformance as prior NDS work.
