// Debug calendar date generation
console.log('DEBUGGING CALENDAR DATE GENERATION');
...
// Test the calendar generation logic
var today = new Date();
...
// Simulate the calendar data structure
var calendarData = {
    year: todayHijri.year,
    month: todayHijri.month,
    daysInMonth: HijriCalendar.hijriMonthLength(todayHijri.year, todayHijri.month)
};

console.log('\nCalendar Data:', calendarData);

// Test the date generation logic
var yearStart = HijriCalendar.hijriYearStart(calendarData.year);
...
for (var m = 1; m < calendarData.month; m++) {
    currentMonthDayOfYear += HijriCalendar.hijriMonthLength(calendarData.year, m);
}
console.log('Current month day of year:', currentMonthDayOfYear);

console.log('\nTesting first 10 days of month generation:');

for (var day = 1; day <= Math.min(10, calendarData.daysInMonth); day++) {
    ...
    var hijriDateBack = HijriCalendar.gregorianToHijri(gregorianDate);
    
    console.log('Day ' + day + ': JD=' + jd.toFixed(2) + 
                ' → Gregorian=' + gregorianDate.toDateString() + 
                ' → Hijri back=' + hijriDateBack.day + '/' + hijriDateBack.month + '/' + hijriDateBack.year +
                ' (showing: ' + hijriDateBack.day + ')');
}
