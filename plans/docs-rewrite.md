...
- **Phase 3 done:** radio, checkbox, chips, tags, featured-icons, link (with the link default flipped to primary), definition-list, tabs, scroll-more. Component fixes found on the way: readonly checkbox/radio/switch keyboard guard + disabled label color, button width fit-content, button group/progress/status, chip width + disabled icon + no opacity, tag width + ghost text on color, card nds-center moved the card, on-color button progress ring, featured-icon dark style vs parent status + neutral fill, definition-list md/sm titles on phones, tabs card padding + RTL Home/End + centered column + vertical divided icon order, scroll-more max width defaults to its container.
...
- **Process for Phase 3 (owner, 2026-09-26, revised same day):** ONE page at a time, no rush. Accuracy and quality beat speed, and each page doubles as a bug hunt on its component. For each page: read the source, write it via `/nds-doc`, run `check-docs.py`, run `doc-check.mjs` and look at both sheets, run a sonnet cold-read and parity agent, fix, then read the page's prose top to bottom (the owner asks for this text-flow review before every commit). Component bugs found on the way are fixed in the component, in their own commit. Then the owner reviews the page (the contact sheets help), and it is committed alone. No parallel agent drafting. Design calls the owner makes become skill rules.
- **Next page:** code. Order is by reuse (owner, 2026-09-26): tier 1 base components, then tier 2 built on them, then tier 3 form fields and complex; see the tracker in TODO.md.
- **Open, owner's calls:** the Neutral button is nearly black on the dark card in dark mode (tokens). Vertical divided tabs keep `.nds-label { text-align: end }`, which right-aligns a wrapped label's second line while the content starts at the left (asked, unanswered). A link's `nds-icon` modifier shares its name with the UI icon class (nothing breaks; left as is).
- **Settled on the way:** Dark areas: `data-theme="dark"` on any element renders it and its subtree in dark mode; status tags ignore `nds-oncolor`. Every builder preview has a Dark mode button that writes `data-theme="dark"` into the copied markup. The preview frame is at least 200px tall and centers its content: a full-width part that shrinks there has a real bug, fixed in the component (tabs: `align-self: stretch`; scroll-more: `--scroll-max-width` defaults to `100%`). Links are primary by default and `nds-neutral` opts out. Overview = a paragraph + "Pick another component when:"; Built-in Features = the component's showcase: keep the original list, fix wording and false facts, never list RTL/dark; Variants rows target one element the build can read (`#id`, `:first-child`, `:not(.x)`); options apply to every item except a one-per-set state (checked radio); combinable rules are combo rows; a JS-only property (indeterminate) is not a builder option; a script-set state (button `data-status`) is documented in the API, not the builder; form fields get `data-harness="form"` only when a rule can fail two or more ways (Validate/Reset show only while a rule is on; a pass shows an inline success alert).
