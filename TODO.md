...
- [ ] **R2.7 TEMPERED — refresh coverage is a docs/framing fix first, not a blanket registration.** Real gap: `core/refresh.md` + CHANGELOG say "the one call" while participation is registry-opt-in and mode-gated. R2.7's "components that do not participate" list is partly wrong: **Accordion = already covered** — its scanner path calls `container.ndsAccordion.refresh()` at `_js/nds-accordion.js:346-348`, adopting new items on a live root without a registry hook (verified 2026-08-11). Fix: name which components participate and which modes they cover, in the doc + CHANGELOG framing (loader.js:986-1044's owner/scanner split is the authoritative model). Per-component registrations are case-by-case ONLY: **Tables = never** (tried & REVERTED, `_js/nds-loader.js:130-137` — server-sorted pages get re-ordered client-side); **no AJAX-filter re-emit** (server owns the result set; NDS is UI-only); Forms registration is a design discussion, and if it lands it flips S6's rubric (2026-08-10 sweep graded four models wrong for answering `Init.refresh` there). Remaining candidates shrink to Autocomplete/Upload/Forms, each needing per-component justification.
