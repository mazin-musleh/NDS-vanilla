...
| **Foundation files (shared by all)** | the `_sass/tokens/` partials, `_sass/themes/_dga.scss`, `_sass/themes/_register.scss`, `_mixins.scss` | Token + mixin sources. Any change is global. Treat as highest-blast-radius surface. |
...
- **Cross-component reach**: does the fix touch surface that OTHER component files consume — shared tokens in `_sass/tokens/`, shared mixins in `_mixins.scss`, shared classes used inside other components (e.g., `.nds-icon` inside `.nds-card`/`.nds-alert`), or compositional patterns (cards extending alert patterns, drawer ↔ sidemenu)? Local-to-file fixes are LOW cost on this axis; fixes that ripple to ≥1 other component are MEDIUM; fixes on base components consumed by many (`_buttons.scss`, `_icons.scss`, `_mixins.scss`, `_sass/tokens/`) are HIGH.
