...
**Write `NDS-PLAN.md`** at the project root: a table (page, route, legacy libraries, NDS target, status), opened with the line `Managed by NDS IQ`. Stop for the dev's review; build nothing until they approve. The review raises project-wide decisions only — asset URL prefix, porting strategy, prior-NDS split, direction/locale, the CSP grant, the topbar and dark mode when in question (§Build), build pacing. Ask each decision in the conversation — its options plus your recommended default, one question at a time — never by pointing the dev at the plan file; the plan records the answers, the conversation asks the questions. Page-scoped decisions wait for that page's build session; note them in the page's row.
...
- One copied unit can mix both treatments: a toolbar's search box and filter wiring are the defaults above — keep them. Its demo-fed surfaces (suggestion chips, sample endpoints, a control backed by data the project does not track) get the fabrication rule — real data or removed. Split the unit; never drop it whole.
...
- **Behavioral pass** — load the page; NDS init warnings are `NDS`-prefixed. Run `NDS.Init.audit()`: it reports the silent failures a clean console misses (unregistered icons, unclaimed containers). `window.NDSInitConfig = { enableLogging: true }` before the NDS scripts makes it automatic during active work. Exercise what the page wires — submit, filter, advance a step. Required fields of more than one component type: test each type empty, one by one — same required mark, different validation code; one passing field proves nothing about the next. Window won't resize? Load the page in a same-origin iframe at mobile width. The project's own headers block the iframe (`X-Frame-Options`, `frame-ancestors`)? Use the channel-you-create rung below — a headless browser sets its viewport directly.
