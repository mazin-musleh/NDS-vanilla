...
**P3 — The source answers first.** Before you ask the dev a question, answer one the dev asks you, propose a fix for a project constraint, or wire page JS — read what the template ships on it. This holds at every step; a fix proposed without opening the source's section is a guess. Where to look:
...
| no runtime anywhere in the project | first setup | approve the latest release as default |
...
4. **All page content lives inside sections, laid out with NDS primitives.** Read `NDS_ROOT/_source/layout/section.md` before authoring any page.
    ...
    - Spacing between stacked elements comes from the primitives' `--gap`, never hand-rolled margins. `.nds-section-body` adds no gap — unwrapped siblings render flush. `nds-flex` stretches children; a standalone button gets its own row or goes full width.
...
- **Never copy `NDS_ROOT/_site/docs-assets/`.** `assets/` is the runtime; `docs-assets/` is the showroom. A `/docs-assets/…` path in copied markup is demo content, not a dependency: point it at your own data or image. One exception: an event skin you actually want — copy that single `docs-assets/events/<event>/` folder and its one script tag.
...
**Write `NDS-PLAN.md`** at the project root: a table (page, route, legacy libraries, NDS target, status), opened with the line `Managed by NDS IQ`. Stop for the dev's review; build nothing until they approve. The review raises project-wide decisions only — asset URL prefix, porting strategy, prior-NDS split, direction/locale, the CSP grant, build pacing. Page-scoped decisions wait for that page's build session; note them in the page's row.
...
- **`NDS-REPORT.md`** (optional, project root) collects what adoption surfaces about NDS itself: a missing method or event (with the banner and grep that found nothing), canon contradicting a rule, a misleading doc, a reproducible bug — and rule gaps your own execution surfaced (a rule you skipped or misread is a finding; the fix belongs in the rules). Entries name NDS version, instructions version, component, and a generic repro — never the project's own markup, routes, or data. Open with the banner naming the repo's issues page; sending is the dev's call.
...
3. **Topbar + main navigation** — `_site/ui-shell/topbar.html` + the mainnav of `header.html`. Brand slot: the project's logo on `.nds-brand-logo`, text span removed unless the logo is a bare mark.
...
The chrome you copied is the docs site's own. Before page #2, walk it control by control under the porting principle: a control the project has no capability for is removed (an empty notification bell is a broken promise); a kept control gets the project's mechanism and values — identity from the session, counts from the API, links to real routes. Hardcoded sample identity renders perfectly and fails only when a second user sees the first one's name. Strip anything asserting a version, credential, or affiliation the project does not hold.
...
**Open a page's build session with its plan row's noted questions** — they were deferred to exactly this moment (§Plan). **Then, before any markup, list the page's parts** — every control and region, from the legacy page or the dev's brief — and match each against `components.yml`. A part the copy source lacks gets its component from the catalog, never a substitute; no match = the custom case. **P1 applies:** "NDS has no X" — yours or the dev's — is available only after the `use_when` search; ~90 components ship, and a close variant usually exists under a name that doesn't obviously match.

**The porting principle: content, flow, and data structure follow the legacy app; NDS improves the UI/UX.** Sorting, filtering, export, counts, validation chrome, responsive behavior are defaults to apply, not questions to ask.
...
**Copy markup that exists.** A page source is a copy source only for markup you can see in it. Liquid tags (loop/if markers between curly braces and percent signs) are never copied — copy that region from the built twin, where they are rendered rows. Markup that exists only as front-matter settings (`hero_title`, `breadcrumb:`) is copied from the built twin too, never reconstructed.

**A copied page's `<script>` block is canon** (P4): it encodes wiring the page needs, often with comments saying why. Edit it point by point against the original — selectors, endpoints, content. Never rewrite it from what you think the page needs; a rewrite silently drops pieces the original still carries (loading resets, bfcache handlers, completion calls).
...
- **Behavioral pass** — load the page; NDS init warnings are `NDS`-prefixed. Run `NDS.Init.audit()`: it reports the silent failures a clean console misses (unregistered icons, unclaimed containers). `window.NDSInitConfig = { enableLogging: true }` before the NDS scripts makes it automatic during active work. Exercise what the page wires — submit, filter, advance a step. Required fields of more than one component type: test each type empty, one by one — same required mark, different validation code; one passing field proves nothing about the next. Window won't resize? Load the page in a same-origin iframe at mobile width.
- **Visual pass** — a clean console proves nothing about layout. Look at the page at desktop and mobile width: spacing present, every icon a glyph, sticky/width behavior real, dark mode correct, the page reading as one design. Measured is not seen — a probed width is reported as measured, never visually verified. Built from a template or example? Its built twin is the visual spec: open `NDS_ROOT/_site/templates/<name>.html` (or `examples/`) beside your page and compare — source shows structure, never spacing or proportions. Look at the twin BEFORE the build too when the composition is new to you: knowing the target look is what makes drift visible. Serve the twin over HTTP (a quick static server on `NDS_ROOT/_site`); never `file://` — its fetches hit cross-origin walls and fill the console with false errors. A difference you chose is a content swap; a difference you didn't is a bug.
