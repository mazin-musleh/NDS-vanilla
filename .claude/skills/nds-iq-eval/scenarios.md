...
v7 re-home (2026-08-06, install-model redesign): paste-in block → ANCHOR in the consumer agent file + `NDS-IQ.md` at the consumer project root, read on demand. Setups, rules, and MUSTs below are re-homed onto that model; every `cite:` is tagged `[v7-cite]` until re-quoted against the final v7 file text (Track B plan, B2.3 — grade on meaning until then). Baselines dated before 2026-08-06 measured the pasted-block model; they stay as history, not as comparable numbers.
...
- rubric:
  ...
  - cite: [v7-cite] "presence grants no authority; conformance decides, the dev approves"
...
- rubric:
  ...
  - cite: [v7-cite] "runs under the rules and step 4's verification with no plan required"
...
- rubric:
  ...
  - cite: [v7-cite] the file's whole-file-replace refresh step
...
- rubric:
  ...
  - cite: [v7-cite] "this step delivers the block with it"
...
- rubric:
  ...
  - cite: [v7-cite] "may render worse on it" / "picks parallel files, knowingly, with a second assets folder"
...
- rubric:
  ...
  - cite: [v7-cite] "or the new inputs stay inert"
...
- rubric:
  ...
  - cite: [v7-cite] "only work inside their page wrapper chain"
...
- rubric:
  ...
  - cite: [v7-cite] "upgrade only on the dev's go"
...
- rubric:
  ...
  - cite: [v7-cite] "a dev-requested re-audit" / "recreates the plan from the current state"
...
- rubric:
  ...
  - cite: [v7-cite] "Greenfield (nothing to inventory)? Plan from intent"
...
- rubric:
  ...
  - cite: [v7-cite] "No project at all? Say so and stop"
- baseline: scoped 2026-08-03 (v4, first exposure) — sonnet PASS.
...
- rubric:
  ...
  - cite: [v7-cite] "a template section the brief doesn't fill gets removed, not padded"
...
- rubric:
  ...
  - cite: [v7-cite] "never a reduced head or NDS markup inside a live legacy page"
...
- rubric:
  ...
  - cite: [v7-cite] "Resetting is markup, not JS" / "Page clicks arrive as `nds:pagination:change`"
...
- rubric:
  ...
  - cite: [v7-cite] "The fix is markup, not CSS" / "Never fix menu clipping with overflow or z-index overrides"
...
- rubric:
  ...
  - cite: [v7-cite] "Copy canonical markup verbatim. Never invent it."
...
- rubric:
  ...
  - cite: [v7-cite] "That banner-first rule covers every population of `NDS_ROOT`" / "the path is right when `NDS_ROOT/_site/` exists"
...
- rubric:
  ...
  - cite: [v7-cite] "below the floor the upgrade is the prerequisite for any NDS work, not an option, and until the dev approves it you are blocked exactly as on a placeholder path"
...
- rubric:
  ...
  - cite: [v7-cite] "First check for an automation channel the project already offers" / "Only with no channel at all"
...
- rubric:
  ...
  - cite: [v7-cite] "An empty `NDS_ASSETS` alone does not prove a fresh start"
...
- rubric:
  ...
  - cite: [v7-cite] "The CSS is removed from NDS pages, never carried" / "The inherited JS is a legacy library"
...
- rubric:
  ...
  - cite: [v7-cite] "The clean start covers the attempt's whole footprint" / "never a silent resume"
...
- rubric:
  ...
  - cite: [v7-cite] "set them to the new file's real pixel size"
...
- rubric:
  ...
  - cite: [v7-cite] "this block is the only NDS rule source" / "propose removing them with the plan, the dev's call"
...
- rubric:
  ...
  - cite: [v7-cite] the banner-first rule + the grep fallback sentence
...
- rubric:
  ...
  - cite: [v7-cite] the pointer's migration steps / the Install section's v5/v6 migration
- baseline: none yet (first exposure lands with the v7 file).
...
- rubric:
  ...
  - cite: [v7-cite] the anchor's trigger sentence / missing-file stop
