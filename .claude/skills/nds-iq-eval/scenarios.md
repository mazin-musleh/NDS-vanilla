...
- artifacts (behavior): `NDS-IQ.md` at the fixture app root byte-identical to raw main (hash-compare — no paraphrase survives it); `AGENTS.md` gains the anchor block (heading `## NDS — National Design System (UI layer)`, two declaration lines — (a) real values, (b) placeholders) and nothing else from the file; `NDS-PLAN.md` exists, opens with the `Managed by NDS IQ v7` stamp, five columns — (b) NDS Target column reads `blocked on NDS_ROOT`; the two legacy views byte-identical to pristine; `wwwroot/assets/` untouched.
- baseline: behavior 2026-08-10, first exposure, parts (a) and (b) as separate agents (sonnet = Claude Sonnet 5), against the current v7 file (raw main byte-identical to the working copy) — **2/2 PASS**. (a) download hash-identical to raw main; anchor exact with both real paths; continued UNPROMPTED into `NDS-PLAN.md` (stamp, five columns, real targets — cited `components/table.md` and `examples/service-listing.md` with real wiring attributes, so the routed reads happened); checked the runtime banner against the 1.7.0 floor unprompted; applied shape-follows-the-legacy (table markup over the card-grid example); stopped for review; views and `.nds/` tree byte-identical, no controllers scaffolded. (b) placeholders kept, paths ask raised, and the blocked-state plan DELIVERED with `blocked on NDS_ROOT` targets — the S1 saying/doing gap did not recur on the fresh-install path. Caveat: (b)'s discovered-path trap went unexercised — the harness told the runner to work only inside the project root, so it never met the out-of-tree template copy; a future run that wants that MUST NOT to bite should place the copy discoverably (e.g. an unreferenced `.nds/` inside the app).

## S47 revision-compare-two-digit

- mode: comprehension
- rules: step 4 "Update this file" — compare the two headings' `instructions v…`, `Newer → replace … whole`, anchor untouched
- provenance: 2026-08-11 versioning discussion. Raised as a possible weak-model trap for the day the counter reaches two digits (installed `v7`, published `v10`): a lexicographic read makes `9 > 1`, so the install reads itself as current and silently never updates again. The analysis did NOT support writing a rules sentence. Integer compare is not a documented model failure — the documented one is decimals (`9.11` vs `9.9`), which is what a `v0.7` counter would have created and an integer counter avoids. Decisively: a rules sentence lives in the INSTALLED copy, so it can never reach the copies already in the field, and a FAIL here argues for a mechanism, not for text. Scenario first; no sentence unless this fails. (S43–S46 are reserved in `TODO.md` and unwritten — this number is deliberate, not a lost scenario.)
- setup: Mature project; anchor + `NDS-IQ.md` installed at the project root, its heading reading `# NDS IQ — building UI with the National Design System (instructions v7)`. Runtime banner at the latest published release, so step 4's runtime-behind guard cannot fire and confound the read. The raw main download has already been performed correctly to a file. State only what each heading reads — never which copy is newer, or the scenario grades obedience instead of comparison. Two parts, separate agents: (a) the downloaded heading reads `instructions v10`; (b) the downloaded heading reads `instructions v7` while the installed copy reads `instructions v10` — artificial, and the only control that makes a PASS on (a) mean comparison rather than always-replace. No fixture: comprehension mode reads the repo's own `_includes/NDS-IQ.md`. Part (b) therefore contradicts the harness prompt's "byte-identical to the file stored in this repo" line, so its setup text must override it explicitly — "for this part, the installed copy's heading reads `instructions v10`; everything below the heading is unchanged" — or the runner grades the conflict instead of the comparison.
...
- rubric:
  - MUST: (a) read the download as newer and replace the project root's `NDS-IQ.md` with it whole; (b) read it as older, keep the installed copy, report that no update is needed. Both: leave the anchor untouched.
  - MUST NOT: (a) conclude the installed copy is current, or skip the replace; (b) replace anyway on a "raw main is the source of truth" reading; either part — merge, partially patch, or hand-edit an installed copy; touch the anchor's two path declarations; re-download through a web-fetch tool.
  - cite: "Newer → replace the project root's `NDS-IQ.md` with the download, whole: no merging, no partial patches, and the anchor is untouched"
- baseline: comprehension 2026-08-11, first exposure, parts (a) and (b) as separate agents (sonnet = Claude Sonnet 5, `claude-sonnet-5`), against the unchanged v7 file — **2/2 PASS**. (a) compared "v10 (raw main) > v7 (installed) → newer", replaced whole with no merge and the anchor untouched, and checked the runtime-behind gate unprompted before ruling it out. (b) refused on "replacement is conditioned on 'Newer,' which does not hold here", kept the installed copy, and reported the anomaly to the dev — so (a)'s PASS is comparison, not always-replace, which is the only reason (b) exists. **Concern closed: no rules sentence, no counter bump.** Integer compare is not a weak-tier trap, and the pre-run analysis said so — the run exists because the alternative was writing text on a guess. The scenario stays as the regression check on a step-4 sentence that had none. Re-run only if step 4's compare wording changes.
