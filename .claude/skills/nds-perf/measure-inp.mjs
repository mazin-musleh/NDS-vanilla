...
// presentation split, plus the style/layout/script/paint breakdown of the click
// window. Same calibration as measure-lcp.mjs (see its header).
//
...
//
// GOTCHA (cost a whole session once): never use page.setRequestInterception()
// here. It silently suppresses every event-timing entry — interactionCount
// reads 0 and the run looks like a fast page instead of a broken rig. Use
// Network.setBlockedURLs for --block, and trust the interactionCount guard below.
import fs from 'fs';
...
const OBS = () => {
  ...
  new PerformanceObserver((l) => {
    for (const e of l.getEntries()) {
      ...
      window.__e.push({
        iid: e.interactionId, start: Math.round(e.startTime), dur: Math.round(e.duration),
...
async function run() {
  ...
  try {
    ...
    for (const sel of CLICKS) {
      ...
      steps.push({ sel, from, to, split: {
        task: (m1.TaskDuration - m0.TaskDuration) * 1000,
        script: (m1.ScriptDuration - m0.ScriptDuration) * 1000,
        style: (m1.RecalcStyleDuration - m0.RecalcStyleDuration) * 1000,
        styleN: m1.RecalcStyleCount - m0.RecalcStyleCount,
        layout: (m1.LayoutDuration - m0.LayoutDuration) * 1000,
        layoutN: m1.LayoutCount - m0.LayoutCount,
      } });
    }
    const { e, count } = await page.evaluate(() => ({ e: window.__e, count: performance.interactionCount }));
    return { e, count, steps };
  } finally {
    await browser.close();
  }
}
...
console.log(`\n${URL}   CPU ${THROTTLE ? `${CPU}x` : 'unthrottled'}${REDUCED ? ' + reduced-motion' : ''}${BLOCK.length ? `  blocked: ${BLOCK.join(' ')}` : ''}   ${rows.length} runs`);
CLICKS.forEach((sel, i) => {
  const per = rows.map((r) => {
    const s = r.steps[i];
    return s && !s.missing ? { inp: worst(r.e, s.from, s.to), split: s.split } : null;
  }).filter(Boolean);
  if (!per.length) { console.log(`\n  ${sel}\n    NOT FOUND on the page`); return; }
  const inps = per.map((p) => p.inp?.dur).filter((d) => d != null);
  const avg = (k) => Math.round(per.reduce((s, p) => s + p.split[k], 0) / per.length);
  const w = per.find((p) => p.inp)?.inp;
  console.log(`\n  ${sel}`);
  console.log(`    INP  median ${med(inps) ?? '—'}ms   runs [${inps.join(', ') || 'none recorded'}]`);
  if (w) console.log(`         one run: input delay ${w.delay} / processing ${w.proc} / presentation ${w.present}`);
  console.log(`    click window (mean of ${per.length}): task ${avg('task')}ms = script ${avg('script')}ms + style ${avg('style')}ms (x${avg('styleN')}) + layout ${avg('layout')}ms (x${avg('layoutN')}) + ${avg('task') - avg('script') - avg('style') - avg('layout')}ms paint/composite/other`);
});
