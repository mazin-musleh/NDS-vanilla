...
Files with the documented two-phase shape pass. Resolved (were the motivating findings for shapes 2-5): `_js/nds-autocomplete.js` (`this._onInput` and siblings → `{ signal }`; the pre-existing fetch controller renamed `fetchAbortController` so the canonical name holds the listener controller), `_js/nds-upload.js` (instance `abortController` + per-cycle `dragAbortController`), `_js/nds-toc.js` `_onScroll`, `_js/nds-dropmenu.js` `destroy()`, `_js/nds-sidemenu.js` `setupScrollPeek`. The shapes stand as regression guards.
