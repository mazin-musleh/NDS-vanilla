...
- **HEAL CITATION**: if a cited `file:line`'s token still sits within a few lines, rewrite the number; if the token is gone, mark the citation stale rather than trusting it.
...
- **Heal citations.** For each cited `file:line` in the entry, confirm the cited token still appears within a few lines. Moved → rewrite the number. Gone → mark the citation stale (`(citation stale — token not found <date>)`) rather than silently trusting it; a stale citation in a carve-out is a candidate Gap for the next run.
