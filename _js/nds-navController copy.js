...
(() => {
    ...
    const state = {
        ...
        getStaticCSS() {
            if (!this._staticCSS) {
                ...
                this._staticCSS = {
                    ...
                    isRTL: root.dir === 'rtl' || rootStyles.direction === 'rtl',
                    safeZoneBuffer: parseInt(rootStyles.getPropertyValue('--nds-dropdown-safeZone')) || 40
                };
            }
        ...
        // Cache for element transition durations (keyed by element reference)
        _durationCache: new WeakMap(),

        getElementDuration(element) {
            if (!element) return 0;
            if (this.prefersReducedMotion) return 0;

            // Check cache first to avoid forced reflow
            let cached = this._durationCache.get(element);
            if (cached !== undefined) {
                return cached > 0 ? this.getTransitionSpeed() : 0;
            }

            // Read and cache transition duration
            const transition = parseFloat(getComputedStyle(element).transitionDuration) || 0;
            this._durationCache.set(element, transition);
    ...
    const utils = {
        ...
        isRTL: () => state.getStaticCSS().isRTL,
        ...
        getElementWidths(...elements) {
            // First pass: read all bounding rects
            const rects = elements.map(el => el?.getBoundingClientRect());
            // Second pass: read all computed styles
            const styles = elements.map(el => el ? getComputedStyle(el) : null);
            // Third pass: compute final widths
        ...
        updateNavMaxWidth() {
            ...
            // Batch all geometric reads first to avoid forced reflows
            const [navWidth, brandWidth, secondaryWidth, minimalWidth] =
                this.getElementWidths(DOM.nav, DOM.brand, DOM.secondary, DOM.minimal);
            const containerWidth = container?.offsetWidth || 0;

            // Batch all computed style reads
            const containerStyles = container ? getComputedStyle(container) : null;
            const children = container ? Array.from(container.children) : [];
            const childStyles = children.map(child => getComputedStyle(child));

            // Now compute values from cached reads
            let padding = 0;
            let gap = 0;
            if (containerStyles) {
            ...
            const contentMaxWidth = containerWidth || (state.getStaticCSS().contentMaxWidth || 1280);
            const constraint = Math.min(navWidth, contentMaxWidth);

            // Use cached child styles for visibility check
            const visibleChildCount = childStyles.filter(style => style.display !== 'none').length;
            const gapCount = Math.max(0, visibleChildCount - 1);

            const used = brandWidth + secondaryWidth + minimalWidth + padding + (gap * gapCount);
        ...
        checkTogglerVisibility() {
            ...
            // Batch all element collections first
            const primaryChildren = DOM.primary ? Array.from(DOM.primary.children) : [];
            const secondaryChildren = DOM.secondary ? Array.from(DOM.secondary.children) : [];

            // Batch all computed style reads to avoid forced reflows
            const allElements = [DOM.primary, DOM.secondary, ...primaryChildren, ...secondaryChildren].filter(Boolean);
            const allStyles = allElements.map(el => getComputedStyle(el));

            // Extract styles from batched reads
            const primaryStyle = DOM.primary ? allStyles[0] : null;
            const secondaryStyle = DOM.secondary ? allStyles[DOM.primary ? 1 : 0] : null;
            const primaryChildStyles = allStyles.slice(DOM.primary && DOM.secondary ? 2 : (DOM.primary || DOM.secondary ? 1 : 0), (DOM.primary && DOM.secondary ? 2 : (DOM.primary || DOM.secondary ? 1 : 0)) + primaryChildren.length);
            const secondaryChildStyles = allStyles.slice(-secondaryChildren.length || allStyles.length);

            // Count visible items using cached styles
            const primaryItemCount = primaryChildren.filter((child, i) =>
                !child.classList.contains('showMore') &&
                primaryChildStyles[i]?.display !== 'none'
            ).length;

            const secondaryItemCount = secondaryChildren.filter((_, i) =>
                secondaryChildStyles[i]?.display !== 'none'
            ).length;

            const hasVisiblePrimary = primaryStyle?.display !== 'none' && primaryItemCount > 0;
            const hasVisibleSecondary = secondaryStyle?.display !== 'none' && secondaryItemCount > 0;
    ...
    const handleDocumentClick = (event) => {
        ...
        const safeZoneBuffer = state.getStaticCSS().safeZoneBuffer;
