...
(() => {
    ...
    // Split components: namespace → injected-bundle key, for components whose eager
    // shell ships in main (and inits via the normal critical/deferred partition)
    // while their deferred behavior half rides an injected bundle. Generated by the
    // build (window.__NDS_SPLIT). Deliberately SEPARATE from __NDS_BUNDLES so a split
    // namespace never enters nsToBundle — its shell must NOT be treated as
    // wholly-injected (that would delay its eager init). The half self-attaches via
    // NDS.<Name>._installBehavior when its bundle executes; the loader only fetches it.
    const SPLIT = window.__NDS_SPLIT || {};
    ...
    function initializeNDS() {
        ...
        // initCriticalBatch.
        let criticalComponents, deferredComponents, injectedGroups, splitBundles;
        ...
        function initCriticalBatch() {
            ...
            if (criticalIndex < criticalComponents.length) {
                ...
                // Split components' behavior halves: fetch their bundle now (post-
                // reveal, low priority) so each half self-attaches via
                // _installBehavior before the user is likely to interact. No init
                // pass — the half grafts onto the shell that already inited above.
                // Idempotent with the injected kick when a split rides the same bundle.
                for (const key of splitBundles) loadBundle(key);
            }
        }
        ...
        deferredComponents = [];
        injectedGroups = {};
        splitBundles = new Set();
        for (const c of COMPONENTS) {
            const present = c.universal || (c.selector && document.querySelector(c.selector));
            if (!present) continue;
            // Split component: its eager shell inits via the normal buckets below;
            // its behavior half rides an injected bundle, fetched post-reveal.
            if (SPLIT[c.name]) splitBundles.add(SPLIT[c.name]);
            if (c.critical) {
    ...
    // Loads a split component's behavior half on demand (the eager shell's traps
    // call this when a user interacts before the post-reveal kick has attached it).
    // Resolves immediately for a non-split namespace. loadBundle idempotency means
    // the trap path and the post-reveal kick share one fetch.
    NDS.loadSplit = (ns) => { const key = SPLIT[ns]; return key ? loadBundle(key) : Promise.resolve(); };
