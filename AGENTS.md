...
**The adoption guide** — `guides/get-started.md` (hosted on the site, shipped in the zip via `_site/guides/`) is the **single source of truth** for consumer-facing NDS rules: its `COPY START`/`COPY END` instructions block is pasted into a consumer project's own `AGENTS.md`/`CLAUDE.md`, so it loads on **every turn** there and must stand alone. It declares `NDS_ROOT` and `NDS_ASSETS` once at the top, each on its own line, and writes every path relative to them — those two lines are the consumer's whole configuration, so don't reintroduce a per-path placeholder. Its workflow assumes the consumer's project **already exists and serves** — NDS is a UI layer, it never scaffolds an app — so there is one workflow, not a new-project/existing-project fork; the steps that only apply when replacing an existing UI (rule #7's parallel files, legacy-library removal) are marked conditional in place. The zip's `README.md` (`scripts/release-template/`) is a human signpost only — no rules live there. `verify()` fails the build if the `NDS_ROOT` declaration or either `COPY` marker goes missing from `guides/get-started.md`, or if any literal path the guide or README references is missing from the zip.
