...
**Never hand-roll the template zip** — `python scripts/mkrelease.py` builds it. A plain `jekyll build` zip ships absolute `/NDS-vanilla/` asset paths that 404 the moment a consumer drops the folder into their own project. The script builds, runs `_plugins/baseurl_cleaner.rb` (paths → depth-relative) then `_plugins/html_compressor.rb`, drops the five docs-site-only files (`playground.html`, `TOKEN-MIGRATION.md`, `llms.txt`, `robots.txt`, `sitemap.xml`), adds `CHANGELOG.md` + `LICENSE` + `README.md` (from `scripts/release-template/` — a human signpost pointing at the in-zip adoption guide), adds `_source/` (`_js/`, `_sass/`, and the four `_data/content/` catalogs — readable source behind the minified bundles), and verifies the result before handing over `dist/nds-vanilla-template-v<version>.zip`.

**The adoption guide** — `guides/get-started.md` (hosted on the site, shipped in the zip via `_site/guides/`) carries the consumer-facing NDS rules: its `COPY START`/`COPY END` instructions block is pasted into a consumer project's own `AGENTS.md`/`CLAUDE.md`, so it loads on **every turn** there and must stand alone. **The block's single source of truth is `_includes/nds-ai-instructions.md`** — clean, unescaped markdown, rendered into the guide's code element via `{% include %}` + the `escape` filter; edit the block THERE, never in the guide's HTML. The guide's `updated` front-matter field means alignment, not edits: the template release its content matches. It stays at that release number until guide/block content describes unreleased template changes; then it moves to the dev line (`X.Y.x`) and the release sweep finalizes it. Page freshness is `last_edit`'s job. It declares `NDS_ROOT` and `NDS_ASSETS` once at the top, each on its own line, and writes every path relative to them — those two lines are the consumer's whole configuration, so don't reintroduce a per-path placeholder. Its workflow assumes the consumer's project **already exists and serves** — NDS is a UI layer, it never scaffolds an app — so there is one workflow, not a new-project/existing-project fork; the steps that only apply when replacing an existing UI (rule #7's parallel files, legacy-library removal) are marked conditional in place. The zip's `README.md` (`scripts/release-template/`) is a human signpost only — no rules live there. **The block is versioned independently of the template**: a plain integer written LITERALLY in two places — the include's heading (`instructions vN`, kept literal so the file reads standalone online without a build) and the guide's green `.nds-code-tags` chip (`vN`) — and the block closes with an `<!-- end NDS instructions -->` marker, so the in-block upgrade workflow (step 4 of "Upgrading NDS") can compare an installed copy against the template's and replace heading-to-marker mechanically, preserving the consumer's two path declarations. **The counter tracks PUBLISHED revisions, not edits**: bump both stamps (same commit) on the first block edit after the current revision is pushed to main (raw main is the block's publish channel, so a push publishes) — further edits before the next push ride the same number. The release-time stamp sweep does NOT touch them. `verify()` fails the build if the `NDS_ROOT`/`NDS_ASSETS` declarations, the version stamp, or the end marker go missing from the include, if the guide stops including it, loses a `COPY` marker, or its green tag disagrees with the heading version, if the rendered page lacks the stamp, or if any literal path the block, guide, or README references is missing from the zip.

**Evolving the ai instructions block** (`_includes/nds-ai-instructions.md`; public name **NDS IQ**, Integration Quality — the umbrella name for the project's AI layer) — drive changes from real integration runs, not speculation, and verify each finding against the source first (audit findings are often already fixed or misread). Highest value is what a weaker model missed and a stronger one inferred: the block must not lean on capability. Extend an existing principle rather than mint a rule per incident, and keep rules component-agnostic — name a specific component only when the fix is important enough to justify it. Never change source to rescue an agent failure; only when the change improves the component itself or fixes a real gap/bug in it. Keep ONE canonical statement per concern, cross-referenced by name from the other moments that need it — the block loads every turn in a consumer project, so duplicates drift and length costs. A real source gap (missing API, canonical markup contradicting a rule) is fixed in the source, never papered over with instruction text. Batch block edits near releases — don't let main's block drift far ahead of the latest published template (agents may read it straight from the repo). After any substantive block edit, propose a `scoped` `nds-ai-eval` run before calling the edit done; at release prep, propose the 3-model `sweep`. The skill owns modes, scenarios, and baselines (`.claude/skills/nds-ai-eval/`) — propose, don't auto-run.

**Growth control — the suite licenses the trim.** Behavior lives in the eval scenarios; the block's text is the smallest thing that makes them pass. Every field incident becomes a scenario first — the sentence it spawns is negotiable later, the scenario never is. Before any sentence lands, walk the cause-removal ladder top-down: source fix → shipped artifact or mechanism (a template, a stamp, a check, a script) → knowledge at the point of copy (banners, canonical markup) → block text; the block holds only what no mechanism above it can absorb, and a mechanism that ships later lets its text shrink under the same gates. Trim only through the gates: pass→remove→still-pass per sentence, and scale per-component knowledge out of the every-turn block into on-demand surfaces (the banner project) behind an eval-gated routing rule. Budget anchor is external: Claude Code warns at 40K characters on the consumer's whole instruction file, and that file belongs to the dev — so the block's budget is ≤30K chars, leaving the dev ~10K of their own headroom. Crossing it triggers a consolidation-only revision — cluster sentences that accreted separately around one concern, rewrite each cluster as one tighter statement, full-suite rerun + 3-model sweep, no new behavior — before anything new lands. (the block sits at ~52K: the trigger is tripped, so the v7 revision in the 1.7.0 cycle is consolidation-first — banner migration for the JS examples, upgrade mechanics on-demand into NDS_ROOT's guide, accreted bullets rewritten.) Never split the block into multiple always-loaded files, never move rules into satellite guides, and never trim rationale clauses speculatively: weak models comply by quoting the why, so rationale is load-bearing until a scenario proves otherwise. Consolidation waits for a cycle boundary (a release or the banner cycle), never mid-cycle on freshly validated text.
