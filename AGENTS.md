...
**Two guides render it**, each via `{% include %}` + the `escape` filter: `guides/get-started.md` (install + session playbook) and `guides/integration-quality.md` (what the system is, revision history). Their green `.nds-code-tags` chips are **Liquid-derived** from the include's heading — never hardcode a version in a guide. A guide's `updated` front-matter field means alignment, not edits: the template release its content matches. It stays at that release number until guide/rules content describes unreleased template changes; then it moves to the dev line (`X.Y.x`) and the release sweep finalizes it. Page freshness is `last_edit`'s job. The workflow assumes the consumer's project **already exists and serves** — NDS is a UI layer, it never scaffolds an app — so there is one workflow, not a new-project/existing-project fork; the steps that only apply when replacing an existing UI (rule #7's parallel files, legacy-library removal) are marked conditional in place. The zip's `README.md` (`scripts/release-template/`) is a human signpost only — no rules live there.

**The rules are versioned independently of the template**, by two stamps that say different things. The **revision counter** is a plain integer written LITERALLY in the include's heading (`instructions vN`, kept literal so the file reads standalone online without a build) — the identity a consumer compares against raw main to know an update is due. The **pairing stamp** on line 3 (`Validated against template X.Y.Z. Works with X.Y.Z or later.`) records what the revision was validated against, plus the templates it runs on. It is a FLOOR, not an exclusive pair: a newer template never invalidates a revision, and a newer revision still serves the same floor. Keep it to one line — a revision reads artifacts its own template introduced (v7 needs the JS banners and the `_source/` page sources), so the floor is simply that template, and any nuance belongs in the rules' own sections, not here. `mkrelease`'s `sweep_pairing_stamp()` moves the pairing number to the release being packaged, before the build, so the zipped copy and both rendered guides carry one number; `verify()` still checks it, catching a deleted or reworded line. The anchor is version-FREE by design — install once, never churns. **The counter tracks PUBLISHED revisions, not edits**: bump it BY HAND on the first edit after the current revision is pushed to main (raw main is the publish channel, so a push publishes) — further edits before the next push ride the same number. No release sweep touches the counter. **The old raw path `_includes/nds-ai-instructions.md` is now a v6-compatible migration pointer**, not the rules: it keeps v6's exact heading, declarations and `<!-- end NDS instructions -->` marker so an installed v5/v6 block's own refresh step lands on it and self-migrates. Its version must match the include's — **bump both in the same commit**. `verify()` fails the build if `NDS-IQ.md` is missing from the zip top level, if the include loses its version stamp, its pairing stamp, its anchor-canon lines or Liquid-free guarantee, if the pointer loses its v6 heading / marker / version match / two declaration lines, if EITHER guide stops including the file, if either rendered guide lacks the revision stamp or carries a pairing stamp older than the release (which is how a stale `--no-build` `_site` gets caught), or if any literal path the rules, guides, or README reference is missing from the zip — the rules' bare `_source/…` / `_site/…` paths included, not just the `NDS_ROOT/`-prefixed ones.

**The floor is the load-bearing number and nothing sweeps it** — it is hand-maintained in the pairing line and restated in the setup section's sub-floor paragraph, and below it the rules block all NDS work. So `verify()` checks it two ways: every version literal outside the pairing slot must BE the floor (a bump that lands in one sentence and not the others is a file that contradicts itself), and the release being packaged must not sit below it (or the zip ships rules that block the very template shipping them). The revision counter gets the same treatment: the plan-file stamp (`Managed by NDS IQ v7`) is its third copy after the heading and the pointer, and must match. **`python scripts/check-release-guards.py` proves these guards still fire** — it breaks the rules file one way per case and asserts the matching guard notices, because the failure that matters is a guard silently becoming a no-op after someone rewords the sentence it keys on. Run it after any edit to `verify()`, `sweep_pairing_stamp()`, or the stamp/anchor/floor sentences themselves.

**Evolving the consumer rules** (`_includes/NDS-IQ.md`; public name **NDS IQ**, Integration Quality — the umbrella name for the project's AI layer) — drive changes from real integration runs, not speculation, and verify each finding against the source first (audit findings are often already fixed or misread). Highest value is what a weaker model missed and a stronger one inferred: the block must not lean on capability. Extend an existing principle rather than mint a rule per incident, and keep rules component-agnostic — name a specific component only when the fix is important enough to justify it. Never change source to rescue an agent failure; only when the change improves the component itself or fixes a real gap/bug in it. Keep ONE canonical statement per concern, cross-referenced by name from the other moments that need it — duplicates drift, and every sentence is read start to finish once a session. A real source gap (missing API, canonical markup contradicting a rule) is fixed in the source, never papered over with instruction text. Batch edits near releases — don't let main's copy drift far ahead of the latest published template (agents may read it straight from the repo). After any substantive edit, propose a `scoped` `nds-iq-eval` run before calling the edit done; at release prep, propose the 3-model `sweep`. The skill owns modes, scenarios, and baselines (`.claude/skills/nds-iq-eval/`) — propose, don't auto-run.

**Growth control — the suite licenses the trim.** Behavior lives in the eval scenarios; the block's text is the smallest thing that makes them pass. Every field incident becomes a scenario first — the sentence it spawns is negotiable later, the scenario never is. Before any sentence lands, walk the cause-removal ladder top-down: source fix → shipped artifact or mechanism (a template, a stamp, a check, a script) → knowledge at the point of copy (banners, canonical markup) → block text; the block holds only what no mechanism above it can absorb, and a mechanism that ships later lets its text shrink under the same gates. Trim only through the gates: pass→remove→still-pass per sentence, and scale per-component knowledge out of the file into on-demand surfaces (the banner project, the `_source/` doc and page sources) behind an eval-gated routing rule. **The ≤30K character ceiling is RETIRED (v7).** It existed because the rules were pasted into the consumer's own instruction file and loaded every turn, against Claude Code's 40K warning on a file that belongs to the dev — the v7 install model moved the rules out of that file, so the dev's headroom is no longer ours to spend and there is no external ceiling to chase. Size is now a **quality outcome, not a budget**: the file is read start to finish, once a session, by the weakest model tier it serves, so length costs comprehension rather than the dev's budget, and the gates above are what hold it down. Never split it into multiple files (one file, one read), never move rules into satellite guides, and never trim rationale clauses speculatively: weak models comply by quoting the why, so rationale is load-bearing until a scenario proves otherwise. A consolidation pass — cluster sentences that accreted separately around one concern, rewrite each cluster as one tighter statement, full-suite rerun + 3-model sweep, no new behavior — waits for a cycle boundary (a release or the banner cycle), never mid-cycle on freshly validated text.

**Release commit** — `_config.yml` `version` + `latest_release` to the real number, `package.json` / `package-lock.json` to match, sweep the `X.Y.x` `since`/`updated` doc stamps, then `ruby _plugins/js_processor.rb` (the bundle banner carries the version). Annotated tag `vX.Y.Z`, then `gh release create` with the zip.

**Release notes are WRITTEN, not extracted** (changed 1.7.0). Pasting the CHANGELOG section produced 13,752 characters and 75 bullets — a wall nobody reads at the moment they most need to. The notes are a summary: a title in house style (`vX.Y.Z — Headline, Headline & lowercase third`), ~6 highlights with a doc link each, one line sweeping up the remainder, then the numbered upgrade steps. Target ~3K characters. `CHANGELOG.md` keeps the full detail, ships in the zip, and the notes link to its section for anyone who wants everything. **The cost of this split is drift** — the two are now written separately and nothing checks them against each other, so an entry added to one after the fact is invisible to the other. Write the notes LAST, from the finished CHANGELOG section, and treat any upgrade step present in one and missing from the other as a bug.
