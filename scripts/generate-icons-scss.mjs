...
const TIER_1_INLINE = [
  ...
  'alert-circle', 'arrow-down-01', 'cancel-circle',
  ...
  'translation', 'user',
  // JS-injected icons
  'arrow-right-02', 'award-05', 'calendar-03', 'clock-01',
...
const MIRRORED = {
  // arrow-01 family: one base glyph pointing down, rotate for every direction.
  'arrow-left-01':  { source: 'arrow-down-01', transform: 'rotate(90deg)'  },
  'arrow-right-01': { source: 'arrow-down-01', transform: 'rotate(-90deg)' },
  'arrow-up-01':    { source: 'arrow-down-01', transform: 'rotate(180deg)' },
  // arrow-02 family
  'arrow-left-02':  { source: 'arrow-right-02', transform: 'scaleX(-1)' },
};
...
const mirroredAliases = Object.entries(MIRRORED).map(([alias, { source, transform }]) => {
  if (!TIER_1_INLINE.includes(source)) {
    console.error(`mirrored alias '${alias}' references unknown source '${source}'`);
    process.exit(1);
  }
  return `.nds-hgi-${alias} { --nds-icon: var(--nds-icon-${source}); transform: ${transform}; }`;
});
